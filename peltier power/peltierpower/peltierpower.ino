// big lookup table (approx 750 entries, from http://playground.arduino.cc/ComponentLib/Thermistor), subtract 238 from ADC reading to start at 0*C. Entries in 10ths of degree i.e. 242 = 24.2*C Covers 0*C to 150*C For 10k resistor/10k thermistor voltage divider w/ therm on the + side.
...
// declare temperature vs PWM table (remember: factor in max current from RC spikes)
// leave for later

// define time
int time = 0;

#define THERM_PIN 0 // use pin A0 to poll thermistor temperature (of thermal mass)
#define FAN_PIN 5 // use for PWM signal to fan
#define HEATER_PIN 6 // use pin 6 (980 Hz) as PWM pin for heater control

void setup() {
  // set pins to input and output
  pinMode(A0, INPUT); // thermistor
  // pinMode(5, OUTPUT); // fan
  // pinMode(6, OUTPUT); // Peltier device
...
void loop() {
  // read temperature from thermistor (taken from http://playground.arduino.cc/ComponentLib/Thermistor)
  int therm;   
  therm=analogRead(THERM_PIN)-238;
  therm=pgm_read_word(&temps[therm]);
  // PWM the Peltier device according to temperature (for later)
  // print the time, temperature (and duty cycle) to serial
  Serial.println("time:");
  Serial.println((millis()-time)/1000);
  Serial.println("temperature (deg. C):");
  Serial.println(double(therm)/10.0);
  
  // record time, temperature, and duty cycle to file (for later)
  delay(1000);
}
